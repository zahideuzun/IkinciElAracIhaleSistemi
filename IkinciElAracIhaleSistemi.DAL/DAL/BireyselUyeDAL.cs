	...
	public class BireyselUyeDAL
	{
		...
		public void BireyselUyeOnay(int id)
		{
			using (AracIhaleContext aracDb = new AracIhaleContext())
			{
				...
				int uyeId = onaylanacakUye.UyeId;
				var onaylanacakUyeTablosu = aracDb.Uyeler.FirstOrDefault(u => u.Id == uyeId);
				onaylanacakUye.IsActive = true;
				onaylanacakUye.IsDeleted = false;
				onaylanacakUyeTablosu.IsActive = true;
				onaylanacakUyeTablosu.IsDeleted = false;
				aracDb.SaveChanges();
			}
		}

		/// <summary>
		/// bireysel uye admin tarafindan eklenirken onayli olarak eklenir. uye kendi kaydoldugunda isactive false olarak alinir ve onaysiz gozukur.
		/// </summary>
		/// <param name="bireyselUye"></param>
		/// <returns></returns>
